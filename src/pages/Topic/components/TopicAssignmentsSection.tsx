...
const TopicAssignmentsSection = ({
    ...
    const [instructionFiles, setInstructionFiles] = useState<any[]>([]);
    const [submissionDraftByAssignment, setSubmissionDraftByAssignment] = useState<any>({});
    const [uploadingAssignmentId, setUploadingAssignmentId] = useState<any>(null);
    const [submissionsByAssignment, setSubmissionsByAssignment] = useState<any>({});
    const [submissionCountByAssignment, setSubmissionCountByAssignment] = useState<any>({});
    const createPanelRef = useRef<any>(null);
    ...
    const visibleAssignments = useMemo(() => {
        ...
        return sortedAssignments.filter((assignment: any) => assignment.visibleToStudents !== false);
    ...
    useEffect(() => {
        ...
        if (canManage) {
            ...
            const unsubscribe = onSnapshot(allSubmissionsQuery, (snapshot: any) => {
                const counts = {};
                snapshot.forEach((submissionDoc: any) => {
        ...
        const unsubscribe = onSnapshot(mySubmissionsQuery, (snapshot: any) => {
            const map = {};
            snapshot.forEach((submissionDoc: any) => {
                const data = submissionDoc.data();
                if (!data?.assignmentId) return;
    ...
    const normalizeFileName = (name: any) => String(name || 'archivo').replace(/[^a-zA-Z0-9._-]/g, '_');

    const mergeFilesWithLimit = (current: any[] = [], incoming: any[] = []) => {
        const accepted: any[] = [];
    ...
    const uploadFilesToStorage = async ({ files, basePath }: any) => {
    ...
    const notifyRejectedFiles = (rejectedBySize: any, rejectedByCount: any) => {
    ...
    const handleInstructionFilesSelected = (event: any) => {
    ...
    const removeInstructionFile = (indexToRemove: any) => {
        setInstructionFiles((prev) => prev.filter((_, index) => index !== indexToRemove));
    };

    const handleSubmissionNoteChange = (assignmentId: any, value: any) => {
        setSubmissionDraftByAssignment((prev: any) => ({
            ...prev,
            [assignmentId]: {
                ...(prev[assignmentId] || {}),
                note: value
    ...
    const handleSubmissionFilesSelected = (assignmentId, event: any) => {
        const files = Array.from(event.target.files || []);
        if (!files.length) return;
    ...
    const removeSubmissionFile = (assignmentId, indexToRemove: any) => {
        setSubmissionDraftByAssignment((prev: any) => ({
            ...prev,
            [assignmentId]: {
                ...(prev[assignmentId] || {}),
    ...
    const updateAssignmentFlag = async (assignmentId, patch: any) => {
        try {
    ...
    const toggleStudentDelivery = async (assignment: any) => {
        if (canManage) return;
        const dueDate = assignment?.dueAt?.toDate ? assignment.dueAt.toDate() : assignment?.dueAt ? new Date(assignment.dueAt) : null;
                ...
                <div ref={createPanelRef} className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <input
                            ...
                            onChange={(event) => setFormData((prev: any) => ({ ...prev, dueAt: event.target.value }))}
                            className="px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 font-semibold text-slate-700 dark:text-slate-200"
                        />
                    </div>
                    ...
                    <textarea
                        ...
                        onChange={(event) => setFormData((prev: any) => ({ ...prev, description: event.target.value }))}
                    ...
                    <div className="mt-3 space-y-2">
                        ...
                        {instructionFiles.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {instructionFiles.map((file, index: any) => (
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                        <button
                            type="button"
                            onClick={() => setFormData((prev: any) => ({ ...prev, visibleToStudents: !prev.visibleToStudents }))}
                        ...
                        <button
                            type="button"
                            onClick={() => setFormData((prev: any) => ({ ...prev, allowLateDelivery: !prev.allowLateDelivery }))}
                                ...
                                {(assignment.instructionFiles || []).length > 0 && (
                                    <div className="mb-4 space-y-2">
                                        ...
                                        <div className="space-y-1.5">
                                            {assignment.instructionFiles.map((file, index: any) => (
                                                <a
                                ...
                                {canManage ? (
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {(submissionDraftByAssignment[assignment.id]?.files || []).length > 0 && (
                                            <div className="space-y-1.5">
                                                {(submissionDraftByAssignment[assignment.id]?.files || []).map((file, index: any) => (
                                        ...
                                        {(mine?.submissionFiles || []).length > 0 && (
                                            <div className="space-y-1.5">
                                                ...
                                                {(mine?.submissionFiles || []).map((file, index: any) => (
                                                    <a
