...
//
// ─── Changes ──────────────────────────────────────────────────────────────────
//  • Accepts onPreviewPaletteApply prop so BrandingSection can push a suggested
//    colour directly into the live preview form without saving to Firestore.
//  • Exposes current form colors via onFormChange so parent can keep track.

import React, { useState, useCallback, useRef, useEffect } from 'react';
import {
  Palette, Save, Upload, ChevronLeft, ChevronRight,
  Check, RefreshCw, RotateCcw, Maximize2, Minimize2, Zap, Info,
} from 'lucide-react';
...
const InstitutionCustomizationView = ({
  ...
  homeUrl = '/home',
  className = '',
  // New: called whenever a palette swatch from BrandingSection is applied.
  // { token, color } — updates the form color live but does NOT auto-save.
  ...
  const isValidHexColor = useCallback((value: any) => {
    if (typeof value !== 'string') return false;
    return /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(value.trim());
  }, []);

  const getPreviewSafeForm = useCallback((candidate, fallback: any) => {
    const source = candidate || {};
    const safeFallback = fallback || DEFAULTS;
    const safe = { ...source };
    COLOR_FIELDS.forEach((field: any) => {
      const nextValue = source[field];
      safe[field] = isValidHexColor(nextValue) ? nextValue : (safeFallback[field] || DEFAULTS[field]);
    });
    return safe;
  }, [isValidHexColor]);
  ...
  // Apply palette colour from BrandingSection (preview-only, no save)
  useEffect(() => {
    if (!previewPaletteApply) return;
    const { token, color } = previewPaletteApply;
    if (!token || !color) return;
    setForm(prev => {
      const next = { ...prev, [token]: color };
  ...
  }, [previewPaletteApply, getPreviewSafeForm]);
  ...
  // Keep preview live on every form change
  useEffect(() => {
    if (!iframeReady || !iframeRef.current) return;
    const safeForm = getPreviewSafeForm(form, lastValidPreviewRef.current);
    lastValidPreviewRef.current = safeForm;
    injectTheme(iframeRef.current, safeForm);
    if (activeToken) injectHighlight(iframeRef.current, activeToken, safeForm[activeToken]);
  ...
  useEffect(() => {
    const handler = (e: any) => { if (e.key === 'Escape' && fullscreen) setFullscreen(false); };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [fullscreen]);

  const handleChange = useCallback((field, value: any) => {
    setForm(prev => {
  ...
  const handleFocus = useCallback((token: any) => {
    ...
    const safeForm = getPreviewSafeForm(form, lastValidPreviewRef.current);
    lastValidPreviewRef.current = safeForm;
    if (iframeRef.current) injectHighlight(iframeRef.current, token, safeForm[token]);
  ...
  return (
    <div className={rootCls} style={{ background: '#e2e8f0' }}>
      ...
      `}>
        {/* Header */}
        <div className={`flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 dark:border-slate-800 shrink-0 ${sidebarOpen ? '' : 'justify-center'}`}>
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 shadow-md transition-colors duration-300" style={{ backgroundColor: form.primary }}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden clean-scrollbar">
          {sidebarOpen ? (
            <div className="px-4 py-4 space-y-4">
              {/* Identity */}
              <section>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2.5">Identidad</p>
                <div className="space-y-2.5">
                  <div>
                    <label className="text-xs font-medium text-slate-600 dark:text-slate-400 block mb-1.5">Nombre de la institución</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-600 dark:text-slate-400 block mb-1.5">URL del logotipo</label>
              ...
              <section>
                <div className="flex items-center justify-between mb-2.5">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Colores de marca</p>
                  <span className="text-[10px] flex items-center gap-1 px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: hexToRgba(form.primary, 0.10) || 'rgba(99,102,241,0.10)', color: form.primary }}>
                    <Zap size={9} /> En vivo
                  </span>
                </div>
              ...
              <section className="bg-slate-50 dark:bg-slate-800/40 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Fijos (no personalizables)</p>
              ...
              {/* Palette swatches */}
              <section>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Paleta actual</p>
                <div className="flex gap-2">
                  {[form.primary, form.secondary, form.accent, form.cardBorder].map((c, i: any) => (
                    <div key={i} title={c} className="flex-1 h-8 rounded-xl border border-black/10 shadow-sm" style={{ backgroundColor: c }} />
                  ))}
                </div>
              </section>
            </div>
        ...
        {/* Footer */}
        <div className={`shrink-0 border-t border-slate-100 dark:border-slate-800 ${sidebarOpen ? 'px-4 py-3' : 'px-2 py-3 flex flex-col items-center gap-2'}`}>
          {sidebarOpen ? (
            <div className="flex gap-2">
              <button onClick={handleReset} className="flex items-center justify-center gap-1.5 flex-1 text-xs font-medium px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                <RotateCcw size={12} /> Restablecer
              </button>
              <button onClick={handleSave} disabled={saving} className="flex items-center justify-center gap-1.5 flex-1 text-xs font-bold px-3 py-2.5 rounded-xl text-white transition disabled:opacity-70 shadow-lg" style={{ ...saveBtnStyle, boxShadow: `0 4px 14px ${hexToRgba(form.primary, 0.35)}` }}>
            ...
            <>
              <button onClick={handleSave} disabled={saving} title="Guardar" className="w-9 h-9 flex items-center justify-center rounded-xl text-white shadow-md transition disabled:opacity-70" style={saveBtnStyle}>
                {saving ? <RefreshCw size={14} className="animate-spin" /> : saved ? <Check size={14} /> : <Save size={14} />}
              </button>
              <button onClick={handleReset} title="Restablecer" className="w-9 h-9 flex items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                <RotateCcw size={14} />
              </button>
            </>
          )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 shrink-0">
          ...
          <button onClick={reloadIframe} title="Recargar vista previa" className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border-2 transition-all duration-200 shrink-0 ${iframeReady ? 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:border-current hover:scale-105 active:scale-95' : 'border-amber-300 bg-amber-50 text-amber-700 animate-pulse'}`}>
            <RefreshCw size={14} className={!iframeReady ? 'animate-spin' : ''} />
            <span className="hidden sm:inline">{iframeReady ? 'Recargar' : 'Cargando…'}</span>
          </button>
          {iframeReady && (
          ...
          <button onClick={() => setFullscreen(f => !f)} title={fullscreen ? 'Salir de pantalla completa (Esc)' : 'Pantalla completa'} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-white dark:hover:bg-slate-800 transition shrink-0">
        ...
        {/* Active-token banner */}
        {activeToken && (
          <div className="flex items-center gap-2 px-4 py-2 text-xs font-medium border-b shrink-0 transition-all" style={{ backgroundColor: hexToRgba(form[activeToken], 0.08) || 'rgba(99,102,241,0.08)', borderColor: hexToRgba(form[activeToken], 0.25) || 'rgba(99,102,241,0.25)', color: form[activeToken] }}>
            <span className="w-3 h-3 rounded-full border-2 border-white shadow-sm animate-pulse shrink-0" style={{ backgroundColor: form[activeToken] }} />
            <span>Editando <strong>{COLOR_TOKENS.find(t => t.id === activeToken)?.label}</strong>{' — '}los elementos destacados en la vista previa son los afectados por este color.</span>
          </div>
        )}
          ...
          <div className="relative shadow-2xl rounded-xl overflow-hidden transition-[width] duration-500 bg-white" style={{ width: currentVP.width, minWidth: viewport === 'desktop' ? '100%' : undefined, height: '100%', minHeight: 600, ...(activeToken ? { outline: `3px solid ${form[activeToken]}`, outlineOffset: '3px', boxShadow: `0 0 0 6px ${hexToRgba(form[activeToken], 0.12)}, 0 20px 60px rgba(0,0,0,0.18)` } : { boxShadow: '0 20px 60px rgba(0,0,0,0.18)' }) }}>
            {!iframeReady && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white dark:bg-slate-950 gap-4">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center shadow-xl" style={{ backgroundColor: hexToRgba(form.primary, 0.12) ?? '#eef2ff' }}>
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1">Ruta: <code className="font-mono">{homeUrl}</code></p>
                </div>
                <button onClick={reloadIframe} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white mt-2 shadow-lg transition active:scale-95" style={{ backgroundColor: form.primary }}>
                  <RefreshCw size={15} /> Reintentar
                </button>
              </div>
            )}
            <iframe key={iframeKey} ref={iframeRef} src={homeUrl} onLoad={handleIframeLoad} className="w-full h-full border-0" style={{ minHeight: 600 }} title="Vista previa en vivo" />
          </div>
        </div>
        ...
        <div className="px-4 py-2 bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between gap-4 shrink-0">
          <p className="text-[11px] text-slate-400 leading-tight">
            {saved ? '✅ Cambios guardados — todos los usuarios verán la nueva apariencia.' : 'Los cambios se aplican en tiempo real. Pulsa Guardar para que sean permanentes.'}
          </p>
          <div className="flex items-center gap-2 shrink-0">
            {fullscreen && (
              <button onClick={() => setFullscreen(false)} className="text-[11px] flex items-center gap-1 text-slate-500 hover:text-slate-700 transition">
                <Minimize2 size={11} /> Salir
              </button>
            )}
            {!sidebarOpen && (
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg text-white transition disabled:opacity-70 shadow-md" style={saveBtnStyle}>
