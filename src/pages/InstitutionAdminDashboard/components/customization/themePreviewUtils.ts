...
export const hexToRgba = (hex, alpha: any) => {
  if (!hex) return null;
  const h = hex.trim();
...
const mix = (baseHex, mixHex_, w: any) => {
...
const buildHighlightCss = (token, color: any) => {
...
export const injectTheme = (iframe, colors: any) => {
  try {
...
export const injectHighlight = (iframe, token, color: any) => {
  try {
    const doc = iframe?.contentDocument;
    if (!doc?.head) return;
