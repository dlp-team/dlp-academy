...
const HomeContent = ({
    ...
    const getShortcutVisualClasses = (itemId, type: any) => {
    ...
    const isSharedForCurrentUser = (item: any) => {
        return isSharedForCurrentUserUtil(item, user);
    };

    const matchesSharedFilter = (item: any) => {
    ...
    const allFoldersForTree = useMemo(() => {
        const merged = (mergeSourceAndShortcutItems as any)({
            sourceItems: (folders || []) as any[],
            shortcutItems: allShortcutFolders as any[]
        }) as any[];
    ...
    const allSubjectsForTree = useMemo(() => {
        const merged = (mergeSourceAndShortcutItems as any)({
            sourceItems: (subjects || []) as any[],
            shortcutItems: allShortcutSubjects as any[]
        }) as any[];
    ...
    const isFolderWithinCurrentTree = (folderId: any) => {
    ...
    const hasSharedAncestorFolder = (folderId: any) => {
    ...
    const isInsideSharedFolderForItem = (item, itemType: any) => {
    ...
    const getFilteredSubjectsForGroup = (groupSubjects: any) => {
                                ...
                                {layoutMode === 'list' && (
                                     <div className="space-y-2 relative">
                                        ...
                                        {(viewMode === 'grid' || viewMode === 'courses' || viewMode === 'tags') && canCreateInCurrentContext && (
                                            <div
                                                onDragOver={(e: any) => { e.preventDefault(); setIsRootZoneHovered(true); }}
                                        ...
                                        {/* Render Folders */}
                                        {!studentMode && viewMode === 'grid' && filteredFolders.map((folder, index: any) => {
                                            const selectionKey = getSelectionKey(folder, 'folder');
                                            const isSelected = selectMode && selectedItemKeys.has(selectionKey);
                                            return (
                                        ...
                                        {/* Render Subjects */}
                                        {groupSubjects && displayedGroupSubjects.map((subject, index: any) => {
                                            const selectionKey = getSelectionKey(subject, 'subject');
                                            const isSelected = selectMode && selectedItemKeys.has(selectionKey);
                                            return (
                                                <ListViewItemComponent
