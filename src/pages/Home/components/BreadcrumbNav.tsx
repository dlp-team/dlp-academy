...
const BreadcrumbNav = ({ 
    ...
    const handleDragOver = (e, targetFolderId: any) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const handleDrop = (e, targetFolderId: any) => {
        e.preventDefault();
        e.stopPropagation();
        setHoveredId(null);
    ...
    const getContainerClasses = (id: any) => {
    ...
    const draggingCanDropHere = (id: any) => {
         const currentId = currentFolder ? currentFolder.id : 'root';
    ...
    return (
        <nav className="flex mt-1 mb-8" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1 md:space-x-2 flex-wrap">
                ...
                {/* DYNAMIC PATH */}
                {breadcrumbPath.map((folder, index: any) => {
