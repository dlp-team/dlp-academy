...
const InstitutionTableRow = ({ school, onOpenDashboard, onEdit, onToggle, onDelete }: InstitutionTableRowProps) => {
    ...
    const handleOpenDashboard = () => {
        if (school?.id) {
            onOpenDashboard(school.id);
        }
    };

    return (
        <tr
            className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer ${isEnabled ? '' : 'opacity-60'}`}
            ...
            tabIndex={0}
            aria-label={`Abrir panel de institución ${school?.name || ''}`.trim()}
        >
