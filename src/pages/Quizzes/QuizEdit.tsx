...
const QuizEdit = ({ user }: any) => {
    ...
    const [questionDeleteConfirm, setQuestionDeleteConfirm] = useState<any>({
        isOpen: false,
        questionIndex: null
    });
    const [hasEditPermission, setHasEditPermission] = useState(false);
    const [ruleContext, setRuleContext] = useState<any>({
    ...
    const parseDateTimeInput = (value: any) => {
    ...
    const normalizeQuestions = (questions: any) => {
        return (questions || []).map((question: any) => ({
            ...
            options: (question?.options || []).map((option: any) => String(option ?? '').trim()),
    ...
    useEffect(() => {
        const fetchQuiz = async () => {
            if (!topicId || !quizId) {
                setSaveError('No se pudo identificar el test para editar.');
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                const topicRef = doc(db, "topics", topicId as string);
                ...
                const topicData: any = { id: topicSnap.id, ...topicSnap.data() };
                ...
                try {
                    ...
                    const subjectRef = doc(db, 'subjects', subjectId as string);
    ...
    // --- HANDLERS ---
    const updateField = (field: any, value: any) => setQuizData({ ...quizData, [field]: value });
    
    const updateQuestion = (idx: any, val: any) => {
        const newQ = [...quizData.questions]; newQ[idx].question = val;
        setQuizData({ ...quizData, questions: newQ });
    };

    const updateOption = (qIdx: any, oIdx: any, val: any) => {
        const newQ = [...quizData.questions]; newQ[qIdx].options[oIdx] = val;
        setQuizData({ ...quizData, questions: newQ });
    };

    const setCorrect = (qIdx, oIdx: any) => {
        const newQ = [...quizData.questions]; newQ[qIdx].correctIndex = oIdx;
        setQuizData({ ...quizData, questions: newQ });
    };

    const requestDeleteQuestion = (questionIndex: any) => {
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 pb-32">
            ...
            <main className="max-w-4xl mx-auto px-6 py-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-6">
                    {quizData.questions.map((q, qIndex: any) => (
                        <div key={qIndex} className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                            ...
                            <div className="p-6 bg-white dark:bg-slate-900 space-y-3">
                                {q.options.map((option, oIndex: any) => {
