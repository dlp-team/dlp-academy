...
const QuizRepaso = ({ user }: any) => {
    ...
    const [answersDetail, setAnswersDetail] = useState<any[]>([]);
    const [masteredInSession, setMasteredInSession] = useState<any[]>([]);
    ...
    const quizData = useMemo(() => ({
        ...
        formulas: [],
        questions: failedQuestions.map((entry: any) => ({
            ...entry,
    ...
    const persistMastered = useCallback(async () => {
        ...
        try {
            ...
            if (error?.code === 'permission-denied') {
                setSaveError('No tienes permiso para guardar tu progreso de repaso.');
            } else {
                setSaveError('No se pudo guardar tu progreso de repaso. Intentalo de nuevo.');
            }
        } finally {
            setSaving(false);
        }
    ...
    const handleAnswerSelect = useCallback((index: any) => {
    ...
    const shouldUseGrid = useMemo(() => {
        ...
        const maxLen = Math.max(...options.map((option: any) => (typeof option === 'string' ? option.length : 0)), 0);
