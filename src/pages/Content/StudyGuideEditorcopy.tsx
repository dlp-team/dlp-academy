...
const FormulaEditor = ({ formula, index, onUpdate, onDelete, topicGradient, baseColor }: any) => {
    return (
...
const SectionEditor = ({ section, index, total, topicGradient, baseColor, onUpdate, onDelete, onMoveUp, onMoveDown, onDuplicate }: any) => {
...
const StudyGuideEditor = ({ user }: any) => {
    ...
    useEffect(() => {
        const load = async () => {
            if (!activeDocId || !topicId || !subjectId) return;
            try {
    ...
    const handleSave = async () => {
        if (!activeDocId || !subjectId || !topicId) {
            showToast('No se pudo identificar la guía activa.', 'error');
            return;
        }
