...
const OnboardingWizard = ({ user }: any) => {
    ...
    useEffect(() => {
        ...
        setStepIndex((prev: any) => {
            if (missing.length === 0) return 0;
            return Math.min(prev, missing.length - 1);
        });
    }, [user]);

    const handleAnswer = async (key, value: any) => {
    ...
    const handleCodeValidation = async (e: any) => {
        ...
        try {
            ...
            const resultData = result?.data as { institutionId?: string } | undefined;

            if (resultData?.institutionId) {
                // ÉXITO
                handleAnswer('institutionId', resultData.institutionId);
            } else {
