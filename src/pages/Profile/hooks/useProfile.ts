...
const fetchTeacherAssignedStudents = async ({ institutionId, teacherUid }: any) => {
    ...
    classesSnapshot.docs.forEach((classDoc: any) => {
        const classData = classDoc.data() || {};
        (classData.studentIds || []).forEach((studentId: any) => {
    ...
    return studentsSnapshot.docs
        .map((studentDoc: any) => ({ id: studentDoc.id, ...studentDoc.data() }))
        .filter((student) => studentIds.has(student.id));
};

const buildStudentCourseAverages = async ({ subjects, assignedStudentIds }: any) => {
    ...
    for (const subject of subjects) {
        ...
        for (const topicDoc of topicsSnapshot.docs) {
            ...
            quizResultsSnapshot.docs.forEach((resultDoc: any) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const subjectsData = querySnapshot.docs.map((d: any) => ({ id: d.id, ...d.data() }));
                ...
                if (resolvedRole === 'teacher' && effectiveInstitutionId) {
                    ...
                    if (teacherStudents.length > 0 && subjectsData.length > 0) {
                        const assignedStudentIds = new Set(teacherStudents.map((student: any) => student.id));
                        const averagesByStudentAndCourse = await buildStudentCourseAverages({
                            subjects: subjectsData,
                            assignedStudentIds
                        });

                        const pendingAutoAwards: any[] = [];
                        averagesByStudentAndCourse.forEach((value, mapKey: any) => {
