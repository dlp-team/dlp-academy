...
const useUserStatistics = (subjects: any[] = [], userId: any, options: any = {}) => {
    ...
    useEffect(() => {
        const fetchDeepStats = async () => {
            ...
            try {
                ...
                const totalScore = allResults.reduce((acc: any, curr: any) => acc + (curr.score || 0), 0);
                ...
                const totalQuestions = allResults.reduce((acc: any, curr: any) => acc + (curr.totalQuestions || 0), 0);
    ...
    const getChartData = (filterSubject: any) => {
        let data: any[] = rawResults;
        if (filterSubject !== 'all') {
            data = rawResults.filter((r: any) => r.subjectId === filterSubject);
        }
        return data.sort((a: any, b: any) => a.date - b.date);
    };

    // Bar color logic
    const getBarGradient = (score: any) => {
        if (score === 100) {
