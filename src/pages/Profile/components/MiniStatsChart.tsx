...
const filterByPeriod = (data: any[] = [], period: any) => {
    if (period === 'all') return data;
    const now = new Date();
    const days = period === '7d' ? 7 : period === '30d' ? 30 : 90;
    const cutoff = new Date(now.getTime() - days * 24 * 60 * 60 * 1000);
    return data.filter((d: any) => d.date >= cutoff);
};

// ─── Detailed Overlay Chart ───────────────────────────────────────────────────
const ChartOverlay = ({ data, role, onClose }: any) => {
    const [period, setPeriod] = useState('all');
    const [hoveredIdx, setHoveredIdx] = useState<any>(null);

    const filtered = useMemo(() => filterByPeriod(data, period).sort((a: any, b: any) => a.date - b.date), [data, period]);
    const scores = filtered.map((d: any) => d.score);
    ...
    const passed = filtered.filter((d: any) => d.score >= 50).length;

    // Sparkline SVG
    const W = 600; const H = 120;
    const points = scores.map((s, i: any) => ({
...
// ─── Mini Card (sidebar) ──────────────────────────────────────────────────────
const MiniStatsChart = ({ data = [], title = 'Evolución de notas', subtitle, role = 'student' }: any) => {
    const [showOverlay, setShowOverlay] = useState(false);

    const scores = data.map((d: any) => d.score);
    const overallAvg = scores.length > 0 ? Math.round(scores.reduce((a: any, b: any) => a + b, 0) / scores.length) : 0;

    const recentScores = scores.slice(-5);
    const avgRecent = recentScores.length > 0 ? recentScores.reduce((a: any, b: any) => a + b, 0) / recentScores.length : 0;
    const prevScores = scores.slice(-10, -5);
    const avgPrev = prevScores.length > 0 ? prevScores.reduce((a: any, b: any) => a + b, 0) / prevScores.length : avgRecent;
        ...
        <>
            ...
            >
                ...
                <div className="flex items-end gap-0.5 h-8">
                    {scores.slice(-20).map((score, i: any) => {
