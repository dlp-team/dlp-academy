...
const ScoreRing = ({ score, size = 48 }: any) => {
...
const Notas = ({
    ...
    const overallAvg = subjectPerformance.length > 0
        ? Math.round(subjectPerformance.reduce((a: any, b: any) => a + b.average, 0) / subjectPerformance.length)
        : null;

    const overall = overallAvg !== null ? gradeInfo(overallAvg) : null;
    const passed  = recentActivity.filter((a: any) => a.passed).length;
