...
const AnnotationBox = ({ annotation, isSelected, onClick }: any) => {
...
const DocumentViewer = ({ documentUrl, annotations, selectedId, onAnnotationClick }: any) => {
  return (
...
const FeedbackPanel = ({ annotation, result }: any) => {
  if (!annotation) {
    return (
...
/** Upload dropzone */
const UploadZone = ({ onFileSelect, isDragging, setIsDragging }: any) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-3">
        {[['image/*', Image, 'Images'], ['.pdf', FileText, 'PDF']].map(([, icon, label]: any) => (
...
// ─── Main Component ────────────────────────────────────────────────────────────
const ExamCorrectionTool = ({ user }: any) => {
