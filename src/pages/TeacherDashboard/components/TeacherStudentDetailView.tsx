...
const TeacherStudentDetailView = ({ user }: any) => {
  const { studentId } = useParams();
  const navigate = useNavigate();

  const [loading, setLoading] = useState(true);
  const [student, setStudent] = useState<any>(null);
  const [sharedClasses, setSharedClasses] = useState<any[]>([]);
  const [sharedCourseCount, setSharedCourseCount] = useState(0);
  const [sharedSubjectNames, setSharedSubjectNames] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const studentData: any = { id: studentSnap.id, ...studentSnap.data() };
        setStudent(studentData);

        const teacherClasses = teacherClassesSnap.docs.map((d: any) => ({ id: d.id, ...d.data() }));
        const filteredSharedClasses = teacherClasses.filter((cl: any) => (cl.studentIds || []).includes(studentId));
        ...
        setSharedClasses(
          filteredSharedClasses.map((cl: any) => ({
            ...cl,
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
          ...
          {sharedSubjectNames.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {sharedSubjectNames.map((name, index: any) => (
