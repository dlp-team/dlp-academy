...
const buildDefaultQuestions = (count) => {
...
const SubjectTestsPanel = ({ user, subject, topics = [] }) => {
    ...
    const quizzesBySection = useMemo(() => {
        const grouped = { basico: [], intermedio: [], avanzado: [] };
                ...
                {sectionQuizzes.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {sectionQuizzes.map((quiz) => {
                            const topicName = topics.find((topic) => topic.id === quiz.topicId)?.name || 'Tema sin nombre';
