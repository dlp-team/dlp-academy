...
const Subject = ({ user }: any) => {
    ...
    // Handlers
    const handleCreateOrRetry = async (data, files: any) => {
        await createTopic(data, files); 
        setShowTopicModal(false);
        setRetryTopicData(null);
    };

    const onRetryTopic = (topic: any) => {
        setRetryTopicData(topic);
        setShowTopicModal(true);
    };
    
    const onDeleteTopicConfirm = (topicId: any) => {
        if (!topicId) return;
    ...
    const handleEditTopicClick = (topic: any) => {
        setEditingTopic(topic);
        setShowEditTopicModal(true);
    };

    // NEW: Handle saving edited topic
    const handleSaveEditedTopic = async (updatedData: any) => {
    ...
    const handleToggleVisibility = async (topicId, makeVisible: any) => {
            ...
            {effectiveIsTeacher && (
                <>
                    <EditSubjectModal
                        ...
                        initialData={subject}
                        onSave={(data: any) => { updateSubject(data); setShowEditModal(false); }}
                    />
