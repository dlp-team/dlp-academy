...
export const useClassMembers = (subject: any) => {
        ...
        const fetchMembers = async () => {
            ...
            try {
                ...
                if (Array.isArray(subject.editorUids)) {
                    subject.editorUids.forEach((uid: any) => {
                ...
                if (Array.isArray(subject.viewerUids)) {
                    subject.viewerUids.forEach((uid: any) => {
                        if (!userRoles.has(uid)) {
                ...
                if (Array.isArray(subject.sharedWith)) {
                    subject.sharedWith.forEach((share: any) => {
                ...
                if (Array.isArray(subject.sharedWithUids)) {
                    subject.sharedWithUids.forEach((uid: any) => {
                        if (!userRoles.has(uid)) {
                            userRoles.set(uid, 'viewer');
                        }
                    });
                ...
                membersList.sort((a: any, b: any) => roleOrder[a.role] - roleOrder[b.role]);
