...
export const useFolders = (user: any) => {
    ...
    const addSubjectToFolder = async (folderId, subjectId: any) => {
        if (!folderId) return;
    ...
    const updateFolder = async (id, payload: any) => {
        const sanitizedPayload = {
    ...
    const deleteFolderOnly = async (id: any) => {
        const folder = folders.find(f => f.id === id);
        if (!folder) return;
    ...
    const shareFolder = async (folderId, email, role = 'viewer') => {
        ...
        try {
            ...
            const foldersRollbackState: any[] = [];
            const subjectsRollbackState: any[] = [];
            ...
            const subtreeFolderIds = new Set<any>([folderId]);
    ...
    const unshareFolder = async (folderId, email: any) => {
        try {
            const emailLower = email.toLowerCase();
            ...
            const cleanupFailures: any[] = [];
            const isMissingShortcutFailure = (scope, error: any) => {
            ...
            const addFailure = (scope, id, error: any) => {
            ...
            const stripShareForUser = (sharedWith: any[] = [], sharedWithUids: any[] = []) => {
            ...
            const stripSubjectShareForUser = (sharedWith: any[] = [], sharedWithUids: any[] = []) => {
                return stripShareForUser(sharedWith, sharedWithUids);
            };

            const updateFolderUnshare = async (targetFolderId: any) => {
                const targetFolderRef = doc(db, 'folders', targetFolderId);
            ...
            const updateSubjectsInFolderUnshare = async (targetFolderId: any) => {
                const subjectsSnap = await getDocs(
                    query(collection(db, 'subjects'), where('folderId', '==', targetFolderId))
                );
            ...
            const queue: any[] = [folderId];
            const visited = new Set<any>([folderId]);
            const subtreeFolderIds = new Set<any>([folderId]);
            const subtreeSubjectIds = new Set<any>();

            const collectSubjectsInFolder = async (targetFolderId: any) => {
                try {
                    const subjectsSnap = await getDocs(
                        query(collection(db, 'subjects'), where('folderId', '==', targetFolderId))
                    );
     ...
     */
    const moveSubjectBetweenFolders = async (subjectId, fromFolderId, toFolderId, options: any = {}) => {
        ...
        let newFolderSharedUids: any[] = [];
        let oldFolderSharedUids: any[] = [];
        let targetFolderSharedWith: any[] = [];
    ...
    const moveFolderToParent = async (folderId, currentParentId, newParentId, options: any = {}) => {
        ...
        const folderRef = doc(db, "folders", folderId);
        let updatePayload: any = { parentId: newParentId || null, updatedAt: new Date() };
    ...
    const moveFolderBetweenParents = async (folderId, fromParentId, toParentId: any) => {
    ...
    const transferFolderOwnership = async (folderId, nextOwnerEmail: any) => {
        try {
