...
const isNotFoundDelete = (error: any) => {
...
export const cascadeDeleteTopicResources = async ({
    ...
    for (const collectionName of normalizedCollections) {
        try {
            ...
            itemsSnapshot.docs.forEach((itemDoc: any) => {
                deleteTasks.push(
                    deleteDoc(doc(db, collectionName, itemDoc.id)).catch((error: any) => {
