...
const getNormalizedRole = (userOrRole: any) => {
...
const deriveClassId = (payload: any = {}) => {
...
const getClassData = async (classId: any) => {
    ...
    const loadPromise = getDoc(doc(db, 'classes', classId))
        .then((snap: any) => (snap.exists() ? { id: snap.id, ...snap.data() } : null))
        .catch(() => null);
...
const isSharedWithUser = (subject: any = {}, user: any = {}) => {
    ...
    return subject.sharedWith.some((entry: any) => {
...
export const canUserAccessSubject = async ({ subject, user }: any) => {
