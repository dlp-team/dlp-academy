                        ...
                        <div className="absolute right-0"> 
                            ...
                            >
                                    {(effectiveShowEditUI || effectiveShowDeleteUI || canShowShortcutVisibility || canShowShortcutDelete || effectiveCanShareFromMenu) ? (
                                        <>
                                            {effectiveShowEditUI && (
                                                <button onClick={(e: any) => { e.stopPropagation(); onEdit(folder); onToggleMenu(null); }} className="w-full flex items-center gap-2 p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-gray-700 dark:text-gray-300 transition-colors" style={{ fontSize: `${14 * menuScale}px` }}>
                                                    <Edit2 size={14 * menuScale} /> Editar
                                                </button>
                                            )}
                                            {effectiveCanShareFromMenu && (
                                                <button onClick={(e: any) => { e.stopPropagation(); onShare(folder); onToggleMenu(null); }} className="w-full flex items-center gap-2 p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-gray-700 dark:text-gray-300 transition-colors" style={{ fontSize: `${14 * menuScale}px` }}>
                                            ...
                                            {canShowShortcutVisibility && (
                                                <button onClick={(e: any) => { e.stopPropagation(); onDelete(folder, isHiddenFromManual ? 'showInManual' : 'removeShortcut'); onToggleMenu(null); }} className="w-full flex items-center gap-2 p-2 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg text-amber-700 dark:text-amber-400 transition-colors" style={{ fontSize: `${14 * menuScale}px` }}>
                                            ...
                                            {canShowShortcutDelete && (
                                                <button onClick={(e: any) => { e.stopPropagation(); onDelete(folder, (isOrphan || disableUnshareActions) ? 'deleteShortcut' : 'unshareAndDelete'); onToggleMenu(null); }} className="w-full flex items-center gap-2 p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-red-600 dark:text-red-400 transition-colors" style={{ fontSize: `${14 * menuScale}px` }}>
                                                    <Trash2 size={14 * menuScale} /> Eliminar
                                                </button>
                                            )}
                                            {!isShortcut && effectiveShowDeleteUI && (
                                                <button onClick={(e: any) => { e.stopPropagation(); onDelete(folder); onToggleMenu(null); }} className="w-full flex items-center gap-2 p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-red-600 dark:text-red-400 transition-colors" style={{ fontSize: `${14 * menuScale}px` }}>
                                                    <Trash2 size={14 * menuScale} /> Eliminar
                                                </button>
                                            )}
                                        </>
                    ...
                    <div className="absolute inset-0 z-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        {isMovedToShared && folder?._movedToFolderId ? (
                            <div className="pointer-events-auto flex items-center gap-2">
                                ...
                                <button
                                    onClick={(e: any) => { e.stopPropagation(); onDelete(folder, 'deleteShortcut'); }}
