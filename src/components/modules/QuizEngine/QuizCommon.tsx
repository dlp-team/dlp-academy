...
export const extractColorFromGradient = (gradient: any) => {
...
export const RenderLatex = React.memo(({ text }: any) => {
    if (!text) return null;
...
export const FormulaDisplay = React.memo(({ formula, size = 'text-lg' }: any) => (
    <div className={`${size} font-serif text-slate-800 dark:text-slate-100 select-text overflow-x-auto py-1`}>
...
export const LoadingSpinner = () => (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex flex-col items-center justify-center">
        ...
        <p className="text-slate-600 dark:text-slate-300 text-sm font-semibold animate-pulse">Cargando tu test...</p>
    </div>
);

export const SubjectIcon = React.memo(({ iconKey, topicGradient }: any) => {
...
export const ProgressBar = React.memo(({ current, total, gradient }: any) => (
    <div className="fixed top-0 left-0 right-0 z-30 h-2 bg-slate-200/50 dark:bg-slate-800/80 backdrop-blur-sm">
        <div 
...
export const QuizFooter = React.memo(({ 
    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 pb-6 pt-4 px-4 bg-gradient-to-t from-slate-50 via-slate-50/95 to-transparent dark:from-slate-950 dark:via-slate-950/95">
        <div className="max-w-3xl mx-auto bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl p-3 rounded-3xl shadow-2xl border border-white/50 dark:border-slate-700/70 flex items-center justify-between gap-4 pl-6">
            <div className="hidden md:flex items-center gap-3">
                {answerStatus === ANSWER_STATUS.CORRECT && (
                    <span className="text-green-600 dark:text-green-300 font-black flex items-center gap-2 text-sm px-4 py-2 rounded-2xl bg-green-50 dark:bg-green-900/30">
                        <CheckCircle2 className="w-5 h-5"/> ¡Correcto!
                    </span>
                )}
                {answerStatus === ANSWER_STATUS.INCORRECT && (
                    <span className="text-red-600 dark:text-red-300 font-black flex items-center gap-2 text-sm px-4 py-2 rounded-2xl bg-red-50 dark:bg-red-900/30">
                        <XCircle className="w-5 h-5"/> Incorrecto
                    </span>
                )}
                {answerStatus === ANSWER_STATUS.IDLE && (
                    <span className="text-slate-500 dark:text-slate-300 font-bold flex items-center gap-2 text-sm px-4 py-2 rounded-2xl bg-slate-100 dark:bg-slate-800">
