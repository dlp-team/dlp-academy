<!-- copilot/plans/active/autopilot-plan-execution-2026-04-10/sources/source-autopilot-user-spec-autopilot-plan-execution-2026-04-10.md -->
ReadMe
...
When selection mode is active, make the crear nueva asignatura button do nothing when clicking, but don’t hide it, because it is actually being hidden/unvisible and it must be visible so it can be used when dragging the selected to move them to another layer..
...
On the home page, on some tabs you can see a “Solo Vigentes” button, this one should only show the current subjects from the current academic year that are active(they must be active on the trimester and not finalized), but it is showing a lot of them.
