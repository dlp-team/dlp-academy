...
| FR-003 | teacher/editor | move subject between folders | `/subjects/{subjectId}` update (`folderId`, sharing fields) | allow for same-institution non-student editors/teachers with valid resource write path | `Missing or insufficient permissions` on legacy docs | legacy subject docs missing creation-era fields failed update guard (`course`,`inviteCode`,`enrolledStudentUids`) | `subjects` update predicate requiring `hasAll([...])` | runtime error trace from `useFolders.js:988` + new rules test | RESOLVED |
...
- Rules hardening outcome:
  ...
  - Relaxed subject update guard for legacy docs by validating creation-era fields only when present on update.
  - Added regression test for legacy subject move/update path.
  - `npm run test:rules`: 41/41 passed.
